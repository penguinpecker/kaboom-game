    ...
    <div className="fixed inset-0 z-[95] modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-60 h-full bg-surface-container-low border-r border-outline-variant/10 p-5 animate-scale-in">
        ...
        <div className="mt-6 pt-4 border-t border-outline-variant/10 space-y-1">
          <button onClick={() => { open("referral"); onClose(); }} className="flex items-center gap-3 px-3 py-1.5 font-headline text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/50 hover:text-primary w-full">
            <span className="material-symbols-outlined text-sm mi">link</span>Referral
          </button>
