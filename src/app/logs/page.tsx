    ...
    <div className="px-6 lg:px-8 pb-16 min-h-screen">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="font-body text-sm text-on-surface-variant max-w-lg">Comprehensive archive of all tactical engagements. Cached locally with on-chain tx verification via Somnia explorer.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2.5fr_1fr] gap-8 mb-8">
        <div className="bg-surface-container-low border border-outline-variant/10 rounded-lg p-6">
          ...
          <div className="grid grid-cols-3 gap-8">
            <div>
              ...
              <span className={`font-headline text-3xl font-bold ${stats.pnl >= 0 ? "text-primary" : "text-error"}`}>{stats.pnl >= 0 ? "+" : ""}{stats.pnl.toFixed(3)} STT</span>
        ...
        <div className="bg-surface-container-low border border-outline-variant/10 rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center"><span className="font-headline text-[10px] text-on-surface-variant uppercase tracking-widest">Min Bet</span><span className="font-headline text-sm font-bold text-on-surface">0.001 STT</span></div>
            <div className="w-full h-1 bg-surface-container-highest rounded"><div className="h-full bg-primary rounded w-[10%]" /></div>
            <div className="flex justify-between items-center"><span className="font-headline text-[10px] text-on-surface-variant uppercase tracking-widest">Max Bet</span><span className="font-headline text-sm font-bold text-on-surface">{maxBet} STT</span></div>
          </div>
        ...
        {paged.length === 0 ? (
          ...
          paged.map((g) => (
            <div key={`${g.gameId}-${g.timestamp}`} className="grid grid-cols-7 px-6 py-4 items-center border-b border-outline-variant/[0.04] hover:bg-surface-container-highest transition-colors">
              ...
              <span className="font-headline text-sm text-on-surface">{g.bet.toFixed(3)} STT</span>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`font-headline text-sm font-bold ${g.won ? "text-primary" : "text-error"}`}>{g.won ? "+" : "-"}{(g.won ? g.payout : g.bet).toFixed(3)} STT</span>
              </div>
