...
export default function VaultPage() {
  ...
  const handleDeposit = () => {
    deposit(fundAmt);
    toast("Depositing " + fundAmt + " STT...", "primary");
  };
  ...
  return (
    <div className="px-6 lg:px-8 pb-16 min-h-screen kinetic-grid">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <StatCard label="Balance" value={fmt(balance) + " STT"} color="border-primary" valColor="text-primary" />
        <StatCard label="Max Bet" value={fmt(maxBet) + " STT"} color="border-secondary" valColor="text-secondary" />
        <StatCard label="Max Payout" value={fmt(maxPayout) + " STT"} color="border-tertiary" valColor="text-tertiary" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
        ...
        <div className="bg-surface-container-low p-6 border border-outline-variant/10 stealth-card">
          ...
          <div className="space-y-2.5">
            ...
            <InfoRow label="Chain" value="Somnia 50312" valueColor="text-primary" />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface-container-low p-5 border border-outline-variant/10">
            ...
            <button onClick={handleDeposit} disabled={isPending || isConfirming}
              ...
              {isPending || isConfirming ? "CONFIRMING..." : "DEPOSIT STT"}
            </button>
          </div>
            ...
            <div className="space-y-2">
              {Object.entries(CONTRACTS).map(([name, addr]) => (
                <div key={name}>
                  ...
                  <a href={`https://shannon-explorer.somnia.network/address/${addr}`} target="_blank" rel="noreferrer" className="font-mono text-[9px] text-primary hover:underline">
