      ...
      <div className="flex justify-between items-end mb-8">
        ...
        <div className="hidden lg:flex gap-4">
          <div className="bg-surface-container-high p-4 border-l-2 border-primary">
            ...
            <div className={`font-headline text-2xl font-bold ${localStats && localStats.totalPnl >= 0 ? "text-primary" : "text-error"}`}>{localStats ? (localStats.totalPnl >= 0 ? "+" : "") + localStats.totalPnl.toFixed(3) : "+0.000"} STT</div>
          </div>
            ...
            ([...leaders!] as any[]).map((entry: any, i: number) => {
              ...
              return (
                <div key={i} className={`grid grid-cols-5 px-6 py-4 items-center border-b border-outline-variant/[0.04] ${isMe ? "bg-primary/5" : "hover:bg-surface-container-highest transition-colors"}`}>
                  ...
                  <span className="font-headline text-sm font-bold text-primary">{Number(formatEther(entry.biggestWin)).toFixed(3)} STT</span>
            ...
            localLeaders.map(([player, stats], i) => {
              ...
              return (
                <div key={i} className={`grid grid-cols-5 px-6 py-4 items-center border-b border-outline-variant/[0.04] ${isMe ? "bg-primary/5" : "hover:bg-surface-container-highest transition-colors"}`}>
                  ...
                  <span className="font-headline text-sm font-bold text-primary">{stats.biggestWin.toFixed(3)} STT</span>
              ...
              {recentWins.length > 0 ? recentWins.map((g) => (
                <div key={`${g.gameId}-${g.timestamp}`} className="px-5 py-3">
                  ...
                  <div className="flex justify-between items-center mt-1">
                    <span className="font-headline text-sm font-bold text-primary">+{g.payout.toFixed(3)} STT</span>
          ...
          <div className="bg-surface-container-low border border-outline-variant/10 rounded-lg p-5">
            ...
            {localStats ? (
              <div className="space-y-3">
                ...
                <StatRow label="Total PnL" value={(localStats.totalPnl >= 0 ? "+" : "") + localStats.totalPnl.toFixed(3) + " STT"} color={localStats.totalPnl >= 0 ? "text-primary" : "text-error"} />
              </div>
