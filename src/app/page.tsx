    ...
    <section className="relative min-h-[870px] flex items-center justify-center overflow-hidden kinetic-grid">
      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col items-center text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1 bg-surface-container-high rounded-full mb-8 border border-outline-variant/15">
          ...
          <span className="font-headline text-[10px] uppercase tracking-[0.2em] text-on-surface-variant">System Online // Somnia Testnet 50312</span>
        </div>
        ...
        <p className="font-body text-lg text-on-surface-variant max-w-2xl mb-12">
          On-chain mines on a 4×4 grid. Provably fair. 6 reactive smart contracts. Embedded wallet auto-sign. Built on Somnia.
        </p>
      ...
      <div className="container mx-auto px-12 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-12">
          <div className="flex flex-col">
            ...
            <span className="font-headline text-2xl font-bold text-primary">{vaultBal ? Number(formatEther(vaultBal)).toFixed(2) : "—"} STT</span>
          </div>
          <div className="flex flex-col">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-surface-container p-8 rounded-lg relative overflow-hidden group hover:bg-surface-container-high transition-all">
          ...
          <div className="p-4 bg-surface-container-lowest rounded border border-outline-variant/10">
            ...
            <div className="flex items-center gap-2"><div className="flex-grow h-1 bg-surface-container-highest overflow-hidden"><div className="h-full bg-primary w-[40%]" /></div><span className="font-headline text-sm font-bold">0.10 STT</span></div>
          </div>
        </div>
        <div className="bg-surface-container p-8 rounded-lg relative overflow-hidden group hover:bg-surface-container-high transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 items-start">
        <div className="lg:col-span-1">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3"><span className="w-1.5 h-1.5 rounded-full bg-primary status-dot" /><span className="font-headline text-[10px] uppercase tracking-widest text-on-surface">{vaultBal ? Number(formatEther(vaultBal)).toFixed(2) : "—"} STT in Vault</span></div>
            ...
            <div className="divide-y divide-outline-variant/5">
              {recent.length > 0 ? recent.map((g, i) => (
                <div key={`${g.gameId}-${g.timestamp}`} className="grid grid-cols-4 px-6 py-4 items-center hover:bg-surface-container-highest transition-colors">
                  ...
                  <span className={`font-headline text-sm font-bold ${g.won ? "text-primary" : "text-error"} text-right`}>{g.won ? "+" : "-"}{(g.won ? g.payout : g.bet).toFixed(3)} STT</span>
                </div>
              )) : (
...
function SomniaFooter() {
  return (
    <footer className="w-full flex flex-col md:flex-row justify-between items-center px-12 py-8 gap-4 bg-surface-container-lowest border-t border-outline-variant/15">
      <span className="font-headline text-[10px] uppercase tracking-widest text-on-surface-variant">© 2026 KABOOM! Kinetic Engine. All Systems Operational. Somnia Testnet.</span>
      <div className="flex gap-8">
        ...
        <a className="font-headline text-[10px] uppercase tracking-widest text-on-surface-variant hover:text-tertiary transition-colors" href="https://dorahacks.io/hackathon/somnia-reactivity" target="_blank" rel="noreferrer">DoraHacks</a>
      </div>
    </footer>
  );
}
